...
The deployed frontend and API share one origin, so call the API with **relative paths** such as `/api/v1/welcome`; see the [runbook](../documentation/setup-and-hosting.md). `VITE_API_BASE_URL` exists for local development only, when the Vite dev server and the API run on separate ports; it must be empty in production. Append exact [API paths](../api-design/README.md). API client uses `AbortController`, a proposed 10-second browser deadline and no automatic retry loop. Render the branded shell immediately; show a calm temporary-unavailability message with manual retry. Provider cold start may outlast the deadline, so a retry can succeed later. Cancel outstanding requests on unmount.
